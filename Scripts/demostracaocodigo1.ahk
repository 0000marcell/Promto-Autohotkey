...
;ptcode(200,0,"TL","015")
gui,show,w500 h500,teste8
OnMessage(0x03,"WindowMove")
return 

change2:
ptcode(100,100,"TL","015")
return 
change:
ptcode(100,100,"TL","010")
return 



ptcode(x,y,prefixpt,modelpt){
	Global
	Gui,ptcode:new
	gui,ptcode:+ownerteste8
	Gui,ptcode:-caption
	controllist:=[]
	ptx:=x,pty:=y
	Gui,add,picture,w300 h300 x%x% y%y%,MAC.EXE.010.bmp
	controllist.insert("MAC.EXE.010.bmp")
	Gui,Add, Picture,w400 h80 x+5 0xE,banner
	controllist.insert("banner")
	...
	Gui,Add, Picture,w80 h30 y+5 0xE ,banner0
	controllist.insert("banner0")
	banner("green","banner0",prefixpt,TextOptions,Font)
	for,each,value in [modelpt]{	
		Gui,Add, Picture,wp hp x+5 0xE ,banner%A_Index%
		valuetbi=banner%A_Index%
		controllist.insert(valuetbi) 
		banner("yellow",valuetbi,value,TextOptions,Font)
	}
	TextOptions:="x0p y0p s25p center  cffffffff r4 Bold"
	TextOptions2:="x0p y50p s100p center  cffffffff r4 Bold"
	...
	for,each,value in list:=db.getvalues("Campos",camptable){
		...
		for,each,value in list2:=db.getvalues("CODIGO,DR",camplist){
			if(A_Index=1)
				Gui,Add, Picture,wp hp x+5 y85 0xE,%camplist%%A_Index%
			else 
	...
	WinGetPos,x,y,w,h,teste8
	fposx:=x+ptx,fposy:=y+pty
	Gui,Show,x%fposx% y%fposy%,ptcode
	;SetTimer,trackwindow,1
	return
} 
WindowMove(){
	Global
	WinGetPos,x,y,w,h,teste8
	WinMove,ptcode,,x+100,y+50	
}
