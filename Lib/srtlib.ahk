...
| st_leftOf(string [, searchFor])                               |
| st_rightOf(string [, searchFor])                              |
| st_between(string [, searchFor1, searchFor2])                 |
...
output:  aaabbbccc
*/
st_insert(insert, into, pos=1)
{
   if (abs(pos) > StrLen(into))
      return 0
   if (pos>0)
      return substr(into, 1, pos-1) . insert . substr(into, pos)
   else if (pos<=0)
      return substr(into " ", 1, pos-1) . insert . substr(into, pos, -1)
}
...
st_overwrite(overwrite, into, pos=1)
{
   ...
   if (pos>0)
      ...
      return substr(into " ", 1, pos-1) . overwrite . SubStr(into " ", ((pos<0) ? pos+StrLen(overwrite) : 0), -1)
}
...
/*
LeftOf (Made by, Bon. Thanks!)
   string    = String to get from
   searchFor = What to look for

Returns substring of string from beginning of string up to, but not including, searchFor
If searchFor is not found, returns entire string
Note: works diffently than RightOf - might seem awkward, but imagine a virtual delimiter at the end of string

Example: st_LeftOf("c:\xyz.txt"", "\")
Outputs:  c:
*/
st_leftOf(string, searchFor)
{
   i := InStr(string, searchFor)
   If (i)
...
/*
RightOf (Made by, Bon. Thanks!)
   string    = String to get from
   searchFor = What to look for

Returns substring of string from searchFor to end of string
If searchFor is not found, returns empty string
Note: works diffently than LeftOf - might seem awkward, but imagine a virtual delimiter at the end of string


Example: st_RightOf("c:\xyz.txt", ".")
Outputs:  txt
*/
st_rightOf(string, searchFor)
{
   i := InStr(string, searchFor)
   If (i)
      Return SubStr(string, i + StrLen(searchFor))
}


/*
Between (Made by, Bon. Thanks! (modified by tidbit))
   string     = String to get from
   searchFor1 = First string to look for
   searchFor2 = Second string to look for

Returns substring of string between searchFors
If searchFor1 is not found, returns empty string
If searchFor2 is not found, returns from searchFor1 to end of string
searchFor2 is optional

Example: st_Between("c:\xyz.txt", "\", ".")
Output:  xyz
*/
st_between(string, searchFor1, searchFor2="")
{
    If (searchFor2 = "")
      searchFor2 := searchFor1
      
    pos1 := InStr(string, searchFor1)
    If (pos1 > 0)
        pos1 += StrLen(searchFor1)
    Else
        pos1 := StrLen(string) + 1
        
    pos2 := InStr(string, searchFor2, 0, pos1 + 1)
    If (pos2 = 0)
        pos2 := StrLen(string) + pos1 + StrLen(searchFor1)
...
contains
   ...
   string  = The text or array* you want to search.
